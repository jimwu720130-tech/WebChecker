...
| `favorites.json` | 常用網站清單（JSON 陣列）。可於 UI 維護，或使用「⭐常用網站設定」頁面上傳／下載 JSON 檔。 |

請將上述檔案視為**環境／使用者資料**，若使用版本控制，可依需求列入 `.gitignore`。

---

## 部署到 Streamlit Cloud：常用網站與資料檔

Streamlit Community Cloud 容器檔案系統為**暫時性**，重啟後會回到 **git clone** 的狀態。

| 資料 | 建議作法 |
|------|----------|
| **常用網站（favorites.json）** | 在 repo 中維護 `favorites.json` 並 **commit／push**，Cloud 下次部署即載入；或進 App → **⭐常用網站設定** 使用「上傳 favorites.json」在線覆寫（覆寫僅在本輪容器內有效，**重啟後仍以 Git 檔為準**）。頁面亦提供「下載目前 favorites.json」方便離線編輯後再上傳或提交 Git。 |
| **排除規則（config.json）** | 同上：提交進 repo；若在 Cloud UI 編輯，重啟後會回到 repo 版本（除非你另行備份）。 |

已不再使用 GitHub Gist／Secrets 同步常用網站。
...
│   └── secrets.toml.example    # （選用）Secrets 樣本；常用網站改由 favorites.json／UI 上傳處理
